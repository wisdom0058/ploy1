...
// -----------------------------------
// Vettore di stringhe con elenco delle risposte corrette
// -----------------------------------
// usata per i tipi di domanda con risposta multipla
//function GetListOfRightAnsLetters(question)
//{
//	var j, value;
//	var lettere = [];
...
//	for (j = 0; j < question.answers.length; j++)
//	{
//		value = question.answers[j].valuation;
//		if (value > 0) {
//			lettere.push(NumberToLetter(j));
//		}
//	}
//	return lettere;
//}
...
function CountAnswers() 
{
	...
	{
		if (quiz.nRight + quiz.nNotValuated === options.numOfQuestions) return true;
	}
	else 
	{
...
function ComputeMarks()
{
	...
	for (var i = 0; i < options.numOfQuestions; i++)   // quiz.questions.length
	{
		...
		if ((typeof question === 'undefined') || (question === null))
		{
			...
			totalPoints += options.noChoicePointsArray[i];
		}
		else
		{
			...
			// calcola il punteggio (points)
			if (question.noChoice)
			{
				question.nPoints = question.blankScore;
				totalPoints += question.blankScore;
			}
			else
			{
				if (question.valid === 1) {
					question.nPoints = question.correctScore;
					totalPoints += question.correctScore;
				}
				else if (question.valid === -1) {
					question.nPoints = question.wrongScore;
					totalPoints += question.wrongScore;
				}
				else if (question.valid === 3) {
					var maxScore = Math.max(question.correctScore, question.wrongScore, question.blankScore);
					var minScore = Math.min(question.correctScore, question.wrongScore, question.blankScore);
					var points = (question.nScore - minScore) / (maxScore - minScore);
					question.nPoints = points;
					totalPoints += points
				}
			}
		}
...
///
/// Get the string of final mark or points
///
function GetFinalResult()
{
	...
	{
		if (options.showFinalMark)
		{
			if (str.length > 0) str += " ";
			str += quiz.mark.toString() + " / " + options.maxmark.toString();
		}
		...
		if ((tipo === QMAKE_VALUATION_NEAR_TO_MARK) && (options.showFinalMark)) {
			// valuation near to mark
			str += " - " + GetRemFromMark(quiz.mark);
		}

		if (options.showFinalPoints) {
			if (str.length > 0) str += " | ";
			str += quiz.points.toFixed(options.numDecimalPlacesForPoints);
		}
