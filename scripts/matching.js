...
function CreateMatchingLists(listaLeft, listLeftHtml, listaRight, orderLeft, orderRight, dropdownIdPrefix, textSelect, hideLeftDropdown)
{
    ...
    for (nMatching = 0; nMatching < listaLeft.length; nMatching++) 
    {
        ...
        if (hideLeftDropdown)
        {
            ...
            textHtml += "<button id='" + idBottonLeft + "' class='btn btn-default dropdown-toggle dropdown-matching qf-matching-class qf-matching-left-class qf-matching-left-selected-item-class' type='button' data-toggle='dropdown'><i>" + textSelect + "</i><span class='caret'></span></button>";
            ...
            for (i = 0; i < listaLeft.length; i++) {
                textHtml += "<li><a href='javascript:void(0);' class='qf-matching-class qf-matching-left-class qf-matching-left-item-class'>" + listaLeft[orderLeft[i]] + "</a></li>";
            }
            textHtml += "</ul>";
            textHtml += "</div>"; // dropdown
        }  
        ...
        // right list
        textHtml +="<div class='dropdown'>";
        textHtml += "<button id='" + idBottonRight + "' class='btn btn-default dropdown-toggle dropdown-matching qf-matching-class qf-matching-right-class qf-matching-right-selected-item-class' type='button' data-toggle='dropdown'><i>" + textSelect + "</i><span class='caret'></span></button>";
        textHtml += "<ul ID='" + idDropdownRight + "' class='dropdown-menu qf-matching-class qf-matching-right-class qf-matching-right-dropdown-class'>";

        for (i = 0; i < listaRight.length; i++) 
        {
            textHtml += "<li><a href='javascript:void(0);' class='qf-matching-class qf-matching-right-class qf-matching-right-item-class'>" + listaRight[orderRight[i]] + "</a></li>";
        }
        textHtml +="</ul>";        
        textHtml +="</div>"; // dropdown
        //////////////////
...
function DisableMatching(dropdownIdPrefix, index)
{
    var idBottonLeftName = "#" + dropdownIdPrefix + "_btnLeft_" + index;
    var idBottonRightName = "#" + dropdownIdPrefix + "_btnRight_" + index;
    DisableChoice($(idBottonLeftName));
    DisableChoice($(idBottonRightName));
}

function ClearMatching(dropdownIdPrefix, index)
{
    var idBottonRightName = $("#" + dropdownIdPrefix + "_btnRight_" + index);

    idBottonRightName.val('');
    idBottonRightName.html(textSelect + "<span class='caret'></span>");
}